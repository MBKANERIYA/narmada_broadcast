...
- **Bug Fix (Template Creation)**: Fixed an issue where creating a new video template via the UI resulted in a text-only template on Meta. The chunked upload was incorrectly sending chunks as `multipart/form-data` (which wraps the binary with boundary strings). Meta accepted the upload and returned a handle, but because the video file was inherently corrupted by the boundaries, Meta silently rejected the video during template creation. Reverted the `uploadChunkToMeta` API call to send raw binary data in the body and `file_offset` as an HTTP header (as strictly required by Meta's Resumable API).
**Why**: 
