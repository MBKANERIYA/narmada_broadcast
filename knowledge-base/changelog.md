...
- **Bug Fix (Template Creation)**: Fixed an issue where creating a new video template via the UI resulted in a text-only template on Meta, or threw an `OffsetInvalidError`. The root cause was twofold: (1) we previously used `multipart/form-data` which corrupted the video file boundaries, and (2) when switching back to raw binary, Node's `fetch` defaulted to `Transfer-Encoding: chunked` because it didn't automatically infer the `Content-Length` from the raw `Buffer`. Meta's Resumable Upload API does not support chunked transfer encoding for this endpoint, resulting in it reading 0 bytes and throwing an offset mismatch on subsequent chunks. Explicitly setting `Content-Length` alongside the `file_offset` header permanently resolves this.
**Why**: 
