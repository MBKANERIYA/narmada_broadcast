...
const migrate = async () => {
  ...
  try {
    ...
    `ALTER TABLE orders ADD COLUMN payment_link VARCHAR(255) AFTER currency`,
    `ALTER TABLE orders ADD COLUMN payment_link_id VARCHAR(255) AFTER payment_link`,
    `ALTER TABLE orders ADD COLUMN shipping_address TEXT AFTER fulfillment_status`,
    `ALTER TABLE orders ADD COLUMN last_reminder_at DATETIME AFTER notes`,
