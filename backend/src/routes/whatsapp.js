...
router.post('/templates', async (req, res) => {
    try {
        const { name, category, language, bodyText, headerImageHandle, headerMediaType, footerText, buttons } = req.body;
...
router.put('/templates/:id', async (req, res) => {
    try {
        const { bodyText, headerImageHandle, headerMediaType, footerText, buttons } = req.body;
        if (!bodyText) return res.status(400).json({ error: 'Body text is required' });
